...
<table class="table table-bordered">
    ...
    if (isset($fromdate) and isset($todate)) {
        $sql = "select * from bill inner JOIN signup on bill.email = signup.email where datetime between '$fromdate' and '$todate' and status ='pending'";
    } else {
        $sql = "SELECT * FROM `bill` inner JOIN signup on bill.email = signup.email WHERE bill.status ='pending'";
    }
